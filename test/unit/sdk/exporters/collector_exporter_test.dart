...
import 'package:opentelemetry/src/sdk/proto/opentelemetry/proto/trace/v1/trace.pb.dart' as pb;
...
void main() {
  ...
  test('sends spans', () async {
    ...
    final expectedBody = pb_trace_service.ExportTraceServiceRequest(resourceSpans: [
      pb.ResourceSpans(
            ...
            pb.InstrumentationLibrarySpans(
                spans: [
                  pb.Span(
                      ...
                      attributes: [pb_common.KeyValue(key: 'foo', value: pb_common.AnyValue(stringValue: 'bar'))],
                      status: pb.Status(code: pb.Status_StatusCode.STATUS_CODE_UNSET, message: null),
                  ...
                  pb.Span(
                      ...
                      attributes: [pb_common.KeyValue(key: 'bool', value: pb_common.AnyValue(boolValue: true))],
                      status: pb.Status(code: pb.Status_StatusCode.STATUS_CODE_UNSET, message: null),
                      ...
                      links: [
                        pb.Span_Link(
                            ...
                            traceState: '',
                            attributes: [
                              pb_common.KeyValue(key: 'longKey', value: pb_common.AnyValue(stringValue: 'I am '))
                            ])
                      ])
                ],
                instrumentationLibrary:
                    pb_common.InstrumentationLibrary(name: 'library_name', version: 'library_version'))
          ])
    ]);
    ...
    sdk.CollectorExporter(fakeUri, httpClient: mockClient).export([span1, span2]);

    verify(mockClient.post(fakeUri,
        body: expectedBody.writeToBuffer(), headers: {'Content-Type': 'application/x-protobuf'})).called(1);
  });
  ...
  test('does not send spans when shutdown', () {
    ...
    sdk.CollectorExporter(fakeUri, httpClient: mockClient)
      ..shutdown()
      ..export([span]);

    verify(mockClient.close()).called(1);
    verifyNever(mockClient.post(fakeUri, body: anything, headers: {'Content-Type': 'application/x-protobuf'}));
  });
}
