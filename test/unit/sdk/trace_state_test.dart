...
void main() {
  ...
  test('create from string with invalid values', () {
    ...
    expect(
        () => testTraceState.get('key_0-1'),
        throwsA(isA<ArgumentError>().having(
            (error) => error.message, 'message', 'Key key_0-1 does not exist in TraceState'))); // Invalid value, comma.
    expect(
        () => testTraceState.get('key&0-2'),
        throwsA(isA<ArgumentError>()
            .having((error) => error.message, 'message', 'Key key&0-2 does not exist in TraceState'))); // Invalid key.
    expect(
        () => testTraceState.get('key_@thisisalotlongerthan13characters'),
        throwsA(isA<ArgumentError>().having((error) => error.message, 'message',
            'Key key_@thisisalotlongerthan13characters does not exist in TraceState'))); // Invalid vendor key.
  ...
  test('remove valid value', () {
    ...
    expect(testTraceState.get('key_0-2'), equals('value@1'));
    expect(
        () => testTraceState.get('key_@vendor'),
        throwsA(isA<ArgumentError>().having((error) => error.message, 'message',
            'Key key_@vendor does not exist in TraceState'))); // Invalid vendor key.
