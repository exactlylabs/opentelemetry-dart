...
import '../../proto/opentelemetry/proto/collector/trace/v1/trace_service.pb.dart' as pb_trace_service;
...
class CollectorExporter implements api.SpanExporter {
  ...
  // CollectorExporter(Uri uri, [http.Client? httpClient]) {
  //   this.uri = uri;
  ...
  Iterable<pb_trace.ResourceSpans> _spansToProtobuf(List<api.Span> spans) {
    ...
    final rsm = <sdk.Resource, Map<api.InstrumentationLibrary, List<pb_trace.Span>>>{};
    for (final span in spans) {
      final il = rsm[(span as sdk.Span).resource] ?? <api.InstrumentationLibrary, List<pb_trace.Span>>{};
      il[span.instrumentationLibrary] = il[span.instrumentationLibrary] ?? <pb_trace.Span>[]
    ...
    for (final il in rsm.entries) {
      ...
      final rs =
          pb_trace.ResourceSpans(resource: pb_resource.Resource(attributes: attrs), instrumentationLibrarySpans: []);
      ...
      for (final ils in il.value.entries) {
        rs.instrumentationLibrarySpans.add(pb_trace.InstrumentationLibrarySpans(
            spans: ils.value,
            instrumentationLibrary: pb_common.InstrumentationLibrary(name: ils.key.name, version: ils.key.version)));
      }
      rss.add(rs);
    }
    return rss;
  ...
  Iterable<pb_trace.Span_Link> _spanLinksToProtobuf(List<api.SpanLink> links) {
    ...
    for (final link in links) {
      ...
      for (final attr in link.attributes) {
        attrs.add(pb_common.KeyValue(key: attr.key, value: _attributeValueToProtobuf(attr.value)));
      }
  ...
  pb_trace.Span _spanToProtobuf(sdk.Span span) {
    ...
    return pb_trace.Span(
      ...
      attributes: span.attributes.keys
          .map((key) => pb_common.KeyValue(key: key, value: _attributeValueToProtobuf(span.attributes.get(key)))),
      status: pb_trace.Status(code: statusCode, message: span.status.description),
      kind: spanKind,
